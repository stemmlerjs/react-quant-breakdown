...
class QuantityBreakdown extends React.Component {
  render() {
    ...
    let total = this.props.chunks.reduce((prev, curr) => ({
              ...
              chunks.map((item, index) => {
                return (
                  <div
                    ...
                    style={{
                      height: "20px",
                      width: `${getChunkWidth(total, item.value)}%`,
                      background: getChunkColour(index)
                    }}
                  />
                );
              })
          ...
          <ul style={styles.processListBreakdown}>
            <li>
              <div style={styles.element}>
                <div style={styles.totalName}>
                  {type == "cost" ? "Total Cost" : "Total Time"}
                </div>
                <div style={styles.divider} />
                <div style={styles.value}>
                  {type == "cost" ? isNaN(total) ? (
                    "$0.00"
                  ) : (
                    `$${total.toFixed(2)}`
                  ) : isNaN(total) ? (
                ...
                <li key={index}>
                  <div style={styles.element}>
                    <div style={styles.name}>
                      <div
                        style={{ background: getChunkColour(index) }}
                        style={styles.circle}
                      />
                      {item.name}
                    </div>
